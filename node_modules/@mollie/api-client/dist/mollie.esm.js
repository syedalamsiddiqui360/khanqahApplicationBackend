...
class ApiError extends Error {
  ...
  static createFromResponse(response) {
    return new ApiError(get(response, 'data.detail', 'Received an error without a message'), get(response, 'data.title'), get(response, 'data.status'), get(response, 'data.field'), cloneDeep(get(response, 'data._links')));
  }
...
class Resource {
  ...
  injectPaginationHelpers(input, list, selfParameters) {
    ...
    if (links.previous != null) {
      const {
        query
      } = parse(links.previous.href, true);
...
var methodHelpers = _objectSpread2({}, commonHelpers, {
   ...
   */
  getImage: function getImage(size = MethodImageSize.size2x) {
    switch (size) {
      case '1x':
...
export { ApiMode, HistoricPaymentMethod, Locale, MandateMethod, MandateStatus, MethodImageSize, MethodInclude, OrderEmbed, OrderLineType, OrderStatus, PaymentEmbed, PaymentMethod, PaymentStatus, RefundEmbed, RefundStatus, SequenceType, SubscriptionStatus, createMollieClient };
